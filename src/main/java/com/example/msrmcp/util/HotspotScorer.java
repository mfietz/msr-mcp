package com.example.msrmcp.util;

import com.example.msrmcp.db.FileChangeDao.FileChangeFrequencyRow;
...
public final class HotspotScorer {
    ...
    public static List<HotspotResult> score(
            List<FileChangeFrequencyRow> candidates,
        ...
        int maxFreq = candidates.stream().mapToInt(FileChangeFrequencyRow::changeFrequency).max().orElse(1);

        // ── Collect valid cyclomatic values for normalisation ───────────────
        List<Integer> validCyclo = candidates.stream()
        ...
        List<Integer> validLoc = candidates.stream()
                .map(r -> metricsMap.getOrDefault(r.filePath(), null))
