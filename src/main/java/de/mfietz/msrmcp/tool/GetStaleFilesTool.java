 ...
 *
 * <p>Returns files that have not been changed for at least N days, ranked by staleness score:
 * norm(daysSinceLastChange) × norm(complexity). Non-Java files use LOC as a complexity proxy.
 *
...
public final class GetStaleFilesTool {
    ...
    private static List<StaleFileResult> score(
            List<StaleRow> rows, Map<String, FileMetricsRecord> metricsMap, long now) {
        ...
        List<Integer> validCyclo =
                computed.stream().map(Computed::cyclo).filter(c -> c > 0).toList();
        ...
        List<Integer> validLoc = computed.stream().map(Computed::loc).filter(l -> l > 0).toList();
